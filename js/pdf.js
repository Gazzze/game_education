  ...
  sections: [
    ...
    },
    {
      ...
      content: [
        ...
        '',
        'KOMPRESI VISUAL:',
        '- CSS akan-change untuk GPU compositing (mengurangi repaints).',
        ...
        '',
        'SIGNAL PROCESSING:',
        '- AnalyserNode FFT size 256: menganalisis spektrum frekuensi audio real-time.',
        '- Web Speech API (TTS): rate 0.85, pitch 1.1 untuk kejelasan ucapan anak.',
        '- BiquadFilterNode (lowpass, fc=3000Hz, Q=0.5): menghaluskan karakter suara synth.',
        '- OscillatorNode dengan envelope ADSR untuk feedback suara yang ekspresif.',
        '',
        'KEAMANAN (SECURITY):',
        ...
        '- Input validation: maxlength, regex, dan type checking sebelum pemrosesan.'
      ]
    },
    {
...
function openPDF() {
  ...
  content.innerHTML = pdfContent.sections.map(sec => `
  ...
  loadJsPDF(() => {
    try {
      ...
      function addText(text, x, yPos, opts = {}) {
        const { size = 10, color = [50, 50, 50], bold = false, wrap = true } = opts;
      ...
      doc.text('Game Edukasi Bahasa Inggris', pageW / 2, 48, { align: 'center' });

      doc.setFontSize(11);
      doc.setTextColor(167, 169, 190);
      doc.text('Sistem Multimedia - Media Pendidikan Interaktif', pageW / 2, 60, { align: 'center' });
      doc.text('Untuk Siswa Usia 9-12 Tahun', pageW / 2, 70, { align: 'center' });
      ...
      doc.text('Versi: 1.0.0    |    Tahun: 2024    |    Platform: Web Browser', margin + 5, y + 16);
      doc.text('Teknologi: HTML5 · CSS3 · JavaScript · Web Audio API · Canvas 2D', margin + 5, y + 24);
      doc.text('Lisensi: Pendidikan - Bebas digunakan untuk keperluan pembelajaran', margin + 5, y + 32);
      y += 45;
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setTextColor(167, 169, 190);
        doc.setFont('helvetica', 'normal');
        doc.text('EnglishQuest © 2024 - Sistem Multimedia - Media Pendidikan Interaktif', margin, pageH - 5);
        doc.text(`Halaman ${i} / ${pageCount}`, pageW - margin, pageH - 5, { align: 'right' });
      }
